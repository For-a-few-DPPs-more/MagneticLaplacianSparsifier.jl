...
@testset verbose = true "Uniform spanning trees" begin
    n = 30
    p = 0.9
    eta = 0.0
    q = 0.0
    ...
    @testset "mtsf mean and variance match DPP's mean and variance" begin
        ...
        MC_edges = zeros(n_MC, 1)
        for i in 1:n_MC
            mtsf = multi_type_spanning_forest(
                rng, meta_g, q; weighted=false, absorbing_node=true, ust=true
            )
            MC_edges[i] = ne(mtsf)
        end
